# Smart Flow AI - Edge AI Water Monitoring System 💧🤖
...
*   **Edge AI Classification:** Uses a Decision Tree model running directly on the ESP32 to classify usage into 5 categories:
    *   🚿 **Shower**
    *   🚰 **Tap** (Hand washing/Dishes)
    *   🚽 **Toilet** (Flush)
    *   ⚠️ **Leak** (Continuous low flow)
    *   💤 **Idle**
...
| **Resistors** | 2 | 1kΩ and 2kΩ (For voltage divider on sensor signal). |
...
Smart-Flow-AI/
├── firmware/
│   ├── main.ino            # The final ESP32 firmware (With OLED + GSM + AI)
│   └── data_gathering.ino  # Script used to collect raw training data
├── model_training/
│   ├── generate_data.py    # Python script to generate synthetic datasets
│   ├── train_model.py      # Python script to train Decision Tree & export rules
│   └── flow_model_rules.txt # The C++ logic generated by the script
├── schematics/
│   └── wiring_diagram.png  # Image of the circuit
└── README.md
```
